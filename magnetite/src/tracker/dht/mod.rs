...
struct Node {
    ...
    last_message_rx_time: SystemTime,
    // time of last correct reply received
    last_correct_reply_time: SystemTime,
    // time of last request
    last_ping_sent_time: SystemTime,
    // how many requests we sent since last reply
    pinged: i32,
}

impl Node {
    fn new(peer_id: TorrentId, peer_addr: SocketAddr) -> Node {
        Node {
            peer_id,
            peer_addr,
            last_message_rx_time: std::time::UNIX_EPOCH,
            last_correct_reply_time: std::time::UNIX_EPOCH,
            last_ping_sent_time: std::time::UNIX_EPOCH,
            pinged: 0,
        }
    }
    ...
    fn confirm(&mut self, confirm: ConfirmLevel) {
        let now = SystemTime::now();
        if !confirm.is_empty() {
            self.last_message_rx_time = now;
        }
        if confirm.is_responding() {
            self.last_correct_reply_time = now;
        }
    }
}

struct BucketManager {
    buckets_v4: BTreeMap<TorrentId, Bucket>,
    buckets_v6: BTreeMap<TorrentId, Bucket>,
}
...
impl Bucket {
    ...
    fn new_node(&mut self, id: TorrentId, ss: SocketAddr, confirm: ConfirmLevel) {
        ...
        self.nodes.push(Node::new(id, ss).with_confirm(confirm));
    }
